...
function pegar_pergunta(callback) {
    let endpoint = `https://serene-garden-00468.herokuapp.com/questions/${questionIndex}`
    $.ajax({
...
function gerar_pergunta(pergunta) { // função para tratar o objeto pergunta da API
    $("#pergunta").html(pergunta.quest)  // insere a pergunta no html
    var resposta_correta = pergunta.correct; // insere o valor resposta correta do objeto na variavel
    var respostas = [];
    respostas.push(pergunta.incorrecta);
    respostas.push(pergunta.incorrectb);
    respostas.push(pergunta.incorrectc); // insere as respostas incorretas do objeto na variavel
